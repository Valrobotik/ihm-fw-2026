...
void match_init() {
    ...
    // Init état switch
    if (digitalRead(TEAM_A_PIN) == LOW) isTeamBlue = true;
    else if (digitalRead(TEAM_B_PIN) == LOW) isTeamBlue = false;
...
void match_read_team() {
    ...
    if (currentStateA != lastTeamA) {
        if (millis() - lastSwitchTime < 200) return; 
...
void match_update() {
    ...
    // 2. RESET PRIORITAIRE 
    // IMPORTANT : On NE sort PAS de EMERGENCY_STOP avec le jumper ! (Seul QX le peut)
    ...
    switch (matchState) {
        ...
        case EMERGENCY_STOP:
            ...
            // On ne fait RIEN d'autre ici, le robot est mort tant qu'il n'est pas reset (QX)
            break;
    }
}
